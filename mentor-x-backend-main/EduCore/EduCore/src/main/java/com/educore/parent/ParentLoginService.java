...
public class ParentLoginService {
    ...
    public ParentStartLoginResponse startLogin(ParentStartLoginRequest request) {
        ...
        String otp = otpService.generateAndSendOtp(student.getParent().getPhone());
        log.info("تم إرسال OTP لولي الأمر {}: {}", student.getParent().getPhone(), otp);
    ...
    public ParentCompleteLoginResponse completeLogin(ParentCompleteLoginRequest request) {
        ...
        otpService.verifyOtp(student.getParent().getPhone(), String.valueOf(request.getOtp()));
        log.info("تم التحقق من OTP بنجاح لولي الأمر: {}", student.getParent().getPhone());

        String token = jwtService.generateToken(student.getParent().getPhone(), "PARENT", student.getId());
        log.info("تم إنشاء JWT Token لولي الأمر: {}", student.getParent().getPhone());
