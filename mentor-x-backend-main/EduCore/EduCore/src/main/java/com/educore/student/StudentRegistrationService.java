...
import java.util.*; // هذا سيحل مشكلة List, ArrayList, HashMap
...
public class StudentRegistrationService {
    ...
    // خريطة مؤقتة لتخزين بيانات الملفات قبل الحفظ
    ...
    public CompleteRegisterResponse completeRegistration(
        ...
        // إضافة روابط الملفات إلى الطالب
...
//        Student savedStudent = studentRepository.save(student);
        // 6. الربط الذكي مع ولي الأمر (Logic التعديل هنا)
        Parent parent = parentRepository.findByPhone(request.getParentPhone())
                .orElseGet(() -> {
                    log.info("إنشاء حساب ولي أمر جديد تلقائياً للرقم: {}", request.getParentPhone());
                    return parentRepository.save(Parent.builder()
                            .phone(request.getParentPhone())
                            .build());
                });

// ربط الطالب بالأب (العلاقة الصحيحة)
...
//        log.info("Parent auto-created with phone: {}", savedStudent.getParentPhone());
    ...
    /**
     * رفع الملفات - الدالة الجديدة فقط
     */
    private Map<String, String> uploadStudentFiles(CompleteRegisterRequest request) {
        ...
        try {
            ...
            // في حالة الخطأ، نحذف أي ملفات تم رفعها
            if (!uploadedUrls.isEmpty()) {
                fileUploadService.deleteFiles(uploadedUrls);
            }
            log.error("Error uploading files for phone {}: {}", request.getPhone(), e.getMessage());
            throw new FileUploadException("فشل في رفع الملفات", "student", request.getPhone(), e);
        }
